...
actor RenderQueue<Request: Sendable> {
    private var pendingRequest: Request?
    private var isProcessing = false
    private var lastProcessTime: TimeInterval = 0
    private let throttleInterval: TimeInterval
    private let renderHandler: @Sendable (Request) async -> Void

    /// 初始化渲染队列
    /// - Parameters:
    ///   - maxFPS: 最大帧率限制（0 = 不限制，30 = 30fps，60 = 60fps）
    ///   - renderHandler: 执行渲染的闭包
    init(maxFPS: Int = 0, renderHandler: @escaping @Sendable (Request) async -> Void) {
        // 根据帧率计算节流间隔
